				...
				PDOKAN_FILE_INFO	DokanFileInfo)
{
	...
	return -errno_to_win32_error(impl->find_files(FileName,FillFindData, DokanFileInfo));
}
...
{
	impl_fuse_context *impl=the_impl;
	if (impl->debug()) FWPRINTF(stderr, L"CreateFile : %s AccessMode: %c%c (%x)\n", FileName,
				...
				PDOKAN_FILE_INFO	DokanFileInfo)
{
	...
	return -errno_to_win32_error(impl->flush_file_buffers(FileName, DokanFileInfo));
}
				...
				PDOKAN_FILE_INFO	DokanFileInfo)
{
	...
	return -errno_to_win32_error(impl->lock_file(FileName,ByteOffset, Length,DokanFileInfo));
}
				...
				PDOKAN_FILE_INFO	DokanFileInfo)
{
	...
	return -errno_to_win32_error(impl->unlock_file(FileName,ByteOffset, Length,DokanFileInfo));
}
				...
				PDOKAN_FILE_INFO	DokanFileInfo)
{
	...
	return -errno_to_win32_error(impl->set_file_attributes(FileName, FileAttributes,DokanFileInfo));
}
...
int do_fuse_loop(struct fuse *fs, bool mt)
{
	...
	//Debug
	dokanOptions->Options |= fs->conf.debug ? DOKAN_OPTION_DEBUG : 0;
	dokanOptions->Options |= fs->conf.debug ? DOKAN_OPTION_STDERR : 0;
